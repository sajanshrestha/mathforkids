...
struct CountingGame {
    ...
    init(numberOfProblems: Int, highestNumberOfItems: Int, emojis: [String]) {
        ...
        for index in 0..<numberOfProblems {
            let randomEmoji = emojis.randomElement()!
            let count = Int.random(in: 1...highestNumberOfItems)
            problems.append(CountingGameProblem(id: index, emoji: randomEmoji, countOfItems: count))
        }
    ...
    struct CountingGameProblem: Identifiable {
